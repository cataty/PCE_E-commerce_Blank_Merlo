<?php
    ...
    <x-layout>
        ...
        <form action="{{ route('cargaEditarBlogpost', ['id' => $blogpost->id]) }}" method="POST" enctype="multipart/form-data">
        ...
        <div>
            ...
            <div class="mb-6">
                ...
                <textarea
                    ...
                    required
                >{{ old('contenido', $blogpost->contenido) }}</textarea>
                @error('contenido')
                    <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-6">
                ...
                >
                    ...
                    @foreach ($categorias as $categoria)
                        <option value="{{ $categoria->categoria_blog_id }}" 
                                        {{ old('categoria', $blogpost->categoria_blog_id) == $categoria->categoria_blog_id ? 'selected' : '' }}>
                                        {{ $categoria->nombre_categoria }}</option> @endforeach
